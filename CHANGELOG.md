...
### Changed
- **GG-107 OAuth Umbau — claude login statt eigener OAuth-Client:** Eigener OAuth PKCE Flow (oauth.rs) entfernt und durch Claude Codes Login-Mechanismus (`claude /login`) ersetzt. Login oeffnet jetzt ein integriertes Terminal, in dem der User sich ueber Claude Code bei Anthropic anmeldet. Token wird aus `~/.claude/.credentials.json` gelesen (nicht mehr in settings.json gespeichert). Kein eigener Token-Speicher, keine Verschluesselung noetig — Claude Code verwaltet die Credentials. `AnthropicOAuth`-Struct aus State und Settings entfernt. `get_claude_usage` vereinfacht auf 3-Stufen-Strategie (In-Memory-Cache, File-Cache, Claude Code Token). Logout loescht die Credentials-Datei. Onboarding Option B zeigt jetzt Hinweis statt eigenem Login-Button. i18n-Texte angepasst (DE/EN).

### Added
- **GG-113 Bug-Sync: Status-Rückmeldung „resolved" ans Portal:** Wenn ein Ticket mit `portal_bug_id` nach Done verschoben wird, meldet `move_ticket` den Status „resolved" automatisch ans VTC-Portal zurück. Fire-and-forget: bei Fehler wird geloggt aber der Move nicht blockiert. Ergänzt die in GG-112 eingeführten Status-Rückmeldungen „in_progress" (Übernehmen) und „rejected" (Ablehnen).
- **GG-112 Bug-Sync Inbox — Einträge sichten statt automatisch importieren:** Neue Inbox-View im Dashboard zum Sichten von Portal-Meldungen vor dem Import. Zeigt alle ungesyncten Einträge mit Typ-Badge (Bug/Feedback/Idee), Titel, Beschreibung, Kategorie, Reporter und Datum. Pro Eintrag drei Aktionen: Übernehmen (erstellt Ticket + meldet "in_progress" ans Portal), Ablehnen (meldet "rejected" ans Portal ohne Ticket), Zurückstellen (bleibt in Inbox). Neuer Sync-Modus "Inbox" (Default) vs "Automatisch" in den Projekt-Einstellungen. Backend: `fetch_inbox_bugs` (nur laden), `accept_inbox_bug` (Ticket erstellen + Portal-Status), `reject_inbox_bug` (Portal ablehnen). Neuer Portal-Endpoint `update_bug_status` für PATCH-Statusmeldungen. Nav-Button führt im Inbox-Modus zur Inbox-View statt direkt zu syncen.
- **GG-111 Bug-Sync: Feedback und Ideen als Tickets importieren:** Der Bug-Sync verarbeitet jetzt neben Bugs auch Feedback und Ideen aus der Portal-API. Neues optionales `type`-Feld im `PortalBug`-Struct ("bug", "feedback", "idea"). Mapping: Bugs → TicketType::Bugfix (prio high), Feedback → TicketType::Docs mit "[Feedback] "-Prefix (prio medium), Ideen → TicketType::Feature mit "[Idee] "-Prefix (prio medium). Abwaertskompatibel: Wenn das `type`-Feld fehlt, wird "bug" angenommen. Frontend-Meldungen auf "Eintraege" statt "Bugs" aktualisiert.
- **GG-109 README Editor in der App statt externer Editor plus HTML-Rendering:** Der Stift-Button im Dashboard oeffnet jetzt einen integrierten Editor (Split-Panel: Markdown-Textarea links, Live-Preview rechts) statt die README im externen Editor zu oeffnen. Speichern schreibt die Datei direkt zurueck ins Projekt. Neue Backend-Commands `read_readme_full` und `save_readme`. Zusaetzlich unterstuetzt der Markdown-Renderer jetzt eingebettetes HTML via Allowlist (p, h1-h6, img, a, strong, em, br, hr, div, span, details, summary, table, thead, tbody, tr, th, td, ul, ol, li, code, pre, blockquote). Unsichere Tags und Attribute (onclick, onerror etc.) werden entfernt. i18n-Keys fuer den Editor in DE/EN hinzugefuegt.
...
### Fixed
- **GG-110 README Editor Absturz bei HTML-Inhalt und kaputte Badge-Bilder:** Zwei Bugs im README Dashboard-Rendering behoben. (1) Absturz beim Speichern: `saveReadmeEditor()` aktualisierte `readmeOriginalContent` nach dem Speichern nicht, sodass `closeReadmeEditor()` einen falschen "Ungespeicherte Aenderungen"-Dialog anzeigte. Zusaetzlich fuehrte CRLF-Normalisierung im Textarea dazu, dass der Vergleich schon ohne Aenderungen fehlschlug. Fix: `readmeOriginalContent` wird nach erfolgreichem Speichern aktualisiert und beim Laden aus dem Textarea-Wert (der bereits CRLF→LF normalisiert) gespeichert. (2) Badge-Bilder: Die CSP (`img-src 'self' data:`) blockierte externe URLs von shields.io und GitHub. HTML-`<img>`-Tags mit externen URLs werden jetzt im Sanitizer durch gestylte Badge-Spans mit dem Alt-Text ersetzt, analog zu Markdown-Bildern.
- **GG-108 README Edit-Button: Pfad kaputt und Windows-Fehlerdialog:** `open_readme` auf Windows baute den Pfad falsch zusammen — manuelles Quoten (`format!("\"{}\"", ...)`) wurde von Rusts `Command::args()` doppelt gequotet, was den Laufwerksbuchstaben-Doppelpunkt korrumpierte (z.B. `G:` → `G`). Manueller Quote-Wrapper entfernt; Pfad wird jetzt als separates `.arg(&readme_path)` uebergeben, sodass Rust das korrekte Windows-Argument-Escaping uebernimmt. Behebt damit auch den nativen Windows-Fehlerdialog (roter X-Dialog), der durch den falschen Pfad ausgeloest wurde.
